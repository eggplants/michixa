...
<Story
  ...
  args={{ episode: { index: 3, title: '道草屋の日常', imageUrls: [] }, onShowAbout: fn(), onShowEpisodeMenu: fn() }}
...
<Story
  ...
  args={{ episode: { index: 'ri', title: 'りれきしょ', imageUrls: [] }, onShowAbout: fn(), onShowEpisodeMenu: fn() }}
  play={async ({ canvas }) => {
...
<Story
  name="Long Title"
  args={{ episode: { index: 99, title: 'とても長いタイトルのエピソードのサンプルテキスト', imageUrls: [] }, onShowAbout: fn(), onShowEpisodeMenu: fn() }}
  play={async ({ canvas }) => {
    await expect(
